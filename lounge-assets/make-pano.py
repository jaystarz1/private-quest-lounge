...
# Dusk/night north must use the day camera. A mirrored photo from another
# direction reverses the skyline and cannot register with Central Park.
# These lighting-only edits share the day photo's aspect and horizon anchor.
...
for scene in ("dusk", "night"):
    aligned = os.path.join(src_dir, "aligned", f"{scene}-north-a46.png")
    if not os.path.isfile(aligned):
        raise FileNotFoundError(f"Aligned Central Park asset required: {aligned}")
    files[scene]["north"] = (aligned, files["day"]["north"][1])
